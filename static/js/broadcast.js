...
class BroadcastManager {
    ...
    async startBroadcast(lectureTopic, courseTitle, courseId = null) {
        try {
            // Get user info from localStorage (preferred) or sessionStorage (fallback)
    ...
    async endBroadcast() {
        ...
        try {
            ...
            if (audioBlob) {
                ...
                formData.append('broadcast_id', this.currentBroadcast.id);
                
                // Try to get user_id from sessionStorage as fallback
                const userId = localStorage.getItem('userId') || sessionStorage.getItem('userId');
                if (userId) {
            ...
            // Save broadcast ID before ending
            const broadcastId = this.currentBroadcast.id;

            // End broadcast
            const result = await api.endBroadcast(broadcastId, audioFilePath);
            ...
            api.leaveBroadcastRoom(broadcastId);

            // Store broadcast ID in result for status checking
            if (result.success) {
